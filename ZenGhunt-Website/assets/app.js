...
document.addEventListener('click',e=>{const b=e.target.closest('.filter');if(b){document.querySelectorAll('.filter').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.category=b.dataset.cat;render()}const q=e.target.closest('[data-q]');if(q){state.query=q.dataset.q;$('#search').value=state.query;render();$('#deals').scrollIntoView({behavior:'smooth'})}if(e.target.id==='searchBtn'){state.query=$('#search').value;render();$('#deals').scrollIntoView({behavior:'smooth'})}});
