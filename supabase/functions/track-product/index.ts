    ...
    try {
      ...
      for (const item of items) {
        if (item && typeof item === "object") {
          if (item["@type"] === "Product") out.push(item as Record<string, unknown>);
          if (Array.isArray(item["@graph"])) {
            out.push(
              ...item["@graph"].filter(
                (x: any) => x?.["@type"] === "Product",
              ),
            );
          }
        }
...
function extract(html: string, pageUrl: URL, store: string): Product {
  ...
  const name =
    cleanText(p.name) ||
    meta(html, "og:title") ||
    cleanText(titleMatch?.[1] || null);

  const imageValue = Array.isArray(p.image) ? p.image[0] : p.image;
  const image = imageValue || meta(html, "og:image");
  const price = parsePrice(
    String(offers.price ?? meta(html, "product:price:amount") ?? ""),
  );
  const currency = String(
    offers.priceCurrency || meta(html, "product:price:currency") || "INR",
  );

  return {
    name,
    image: image ? new URL(String(image), pageUrl).href : null,
    price,
    currency,
    store,
    url: pageUrl.href,
...
Deno.serve(async (req) => {
  ...
  if (req.method !== "POST") {
    return new Response(
      JSON.stringify({ error: "POST a product URL" }),
      {
        status: 405,
  ...
  try {
    ...
    if (html.length > 8_000_000) {
      throw new Error("Product page is too large to process.");
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        note:
          product.price == null
            ? "Product page was reachable, but no reliable price was exposed in structured/meta data."
            : null,
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
